...
## Интерфейс
Поле «Оригинал», поле «Перевод» и область «Мини‑промпт» имеют подсказки-placeholder'ы, задаваемые в [`app/ui_main.py`](app/ui_main.py). Толщина неоновой рамки регулируется параметром `neon_width` в настройках, а минималистичный тёмный стиль определяется правилами из [`app/styles.py`](app/styles.py).

- **Поля оригинала и перевода.** Два текстовых поля расположены в горизонтальном разделителе; его можно перетаскивать, меняя пропорции. Поля автоматически растягиваются при изменении размеров окна.
...
Все разделители позволяют скрывать панели вплоть до минимального размера и возвращать их обратно. При изменении размеров окна редакторы, панель глоссария и проектное дерево масштабируются автоматически, явных максимальных ограничений не задано.
