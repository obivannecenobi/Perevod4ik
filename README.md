...
## Версия и обновления
Номер сборки формируется автоматически на основе количества коммитов в репозитории — см. функцию `get_version()` в [`app/__init__.py`](app/__init__.py). Это позволяет распространять обновления без ручного изменения версии.

```python
from app import __version__
print(__version__)  # 0.1.N, где N – число коммитов
```

Проверку наличия свежих коммитов и их загрузку можно выполнить через утилиты из [`app/services/versioning.py`](app/services/versioning.py).
...
## Интерфейс
### Placeholder-поля
Поле «Оригинал», поле «Перевод» и область «Мини‑промпт» имеют подсказки-placeholder'ы, задаваемые в [`app/ui_main.py`](app/ui_main.py).

### Масштабирование
Редакторы, панель глоссария и дерево проектов автоматически растягиваются при изменении размеров окна, а разделители позволяют менять пропорции и скрывать панели при необходимости. Явных максимальных ограничений по размеру не задано.

- **Поля оригинала и перевода.** Два текстовых поля расположены в горизонтальном разделителе; его можно перетаскивать, меняя пропорции.
- **Мини‑промпт.** Находится под редакторами в вертикальном разделителе. Высоту области можно изменять, минимальный размер ограничен содержимым.
- **Таймеры и счётчики.** В нижней панели отображаются секундомер и количество символов; элементы подстраиваются под ширину окна.
- **Глоссарий.** Таблица глоссария располагается справа и добавляется в основной горизонтальный разделитель. Панель можно свернуть, а её ширина настраивается перетаскиванием границы.
- **Панель проектов.** Слева закреплена док‑панель с деревом проектов и кратким описанием. Внутренний разделитель позволяет менять ширину дерева (по умолчанию 200 px) и области описания.

Все разделители позволяют скрывать панели вплоть до минимального размера и возвращать их обратно.

## Проекты и аналитика
Слева располагается панель проектов с иконками и описанием. Список и архив проектов ведёт [`app/project_manager.py`](app/project_manager.py), а метаданные глав и отчёты сохраняются через [`app/services/project.py`](app/services/project.py) и [`app/services/reports.py`](app/services/reports.py). Пример создания проекта:
...
pm = ProjectManager()
proj = pm.create("Demo")
pm.archive(proj.id, archived=True)
```
...
### Автовыбор модели Gemini
`GeminiTranslator` автоматически запрашивает последнюю доступную модель семейства, если явное имя не указано. Логика выбора реализована в [`app/models/gemini.py`](app/models/gemini.py):

```python
from app.models.gemini import GeminiTranslator

translator = GeminiTranslator(api_key=settings.gemini_key)
result = translator.translate("Hello")
```

Функция `fetch_latest_model` получает список моделей из API и кэширует результат, чтобы при следующем запуске использовать актуальную версию.
...
## Выбор шрифтов
Шрифты `Cattedrale` и `Inter` поставляются в каталоге [`assets/fonts`](assets/fonts) и подключаются при старте приложения. Если Qt не удаётся загрузить файлы, используется системный шрифт.

### Установка
- **Linux:** скопируйте файлы `.ttf` в `~/.local/share/fonts` и выполните `fc-cache -f -v`.
- **Windows:** щёлкните правой кнопкой по файлу и выберите «Установить» или перенесите его в `C:\Windows\Fonts`.

Семейство и размер шрифта пользователь выбирает через объект [`AppSettings`](app/settings.py).

```python
from app.settings import AppSettings

settings = AppSettings.load()
settings.base_font = "Inter"
settings.font_size = 12
settings.save()
```

Раздел настроек также позволяет менять цветовую тему и параметры неонового свечения (`neon_width`, `neon_color`).

## Иконки
В корне проекта размещены иконки интерфейса:
- [сохранить.png](сохранить.png)
- [настройки.png](настройки.png)
- [свернуть.png](свернуть.png)
- [лого 2.png](<лого 2.png>)

Иконки допускаются в форматах `.png` и `.ico`. Рекомендуемый размер — 24×24 px или кратный ему.

Чтобы заменить иконку, обновите файл и скорректируйте путь в [`app/ui_main.py`](app/ui_main.py) или соответствующем ресурсном файле Qt, например:
...
Толщина неоновой рамки регулируется параметром `neon_width` в настройках, а минималистичный тёмный стиль определяется правилами из [`app/styles.py`](app/styles.py). Интерфейс оформляется через QSS: элементам задаются скругления 6 px и базовые отступы, что обеспечивает аккуратное размещение виджетов. Основной текст набран шрифтом `Inter`, а заголовки используют декоративный `Cattedrale`.
...
Скрипт [`run.py`](run.py) автоматически устанавливает зависимости, указанные в `requirements.txt`. Запустите его перед первым стартом приложения:
