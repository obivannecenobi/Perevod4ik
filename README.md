...
## Выбор шрифтов
В корне репозитория находятся шрифты `Cattedrale` и `Inter`, которые подключаются при старте приложения. Семейство и размер шрифта пользователь выбирает через объект [`AppSettings`](app/settings.py).
